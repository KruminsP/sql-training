...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `select round(sum(budget_adjusted),2) as total_budget, round(sum(revenue_adjusted),1) as total_revenue
      from movies;`;
  ...
  it(
    ...
    async done => {
      const query = `select original_title, revenue, budget
      from movies
